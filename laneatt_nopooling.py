...
from torchvision.models import resnet18, resnet34, alexnet, vgg16_bn
...
class LaneATT(nn.Module):
    def __init__(self,
        ...
        # self.anchors: 2784, 5+S
        # self.anchors_cut: 2784, 5+11
        ...
        if anchors_freq_path is not None:
            ...
            self.anchors = self.anchors[ind]    # 1000, 5+S
            self.anchors_cut = self.anchors_cut[ind]    # 1000, 5+11
        ...
        self.conv1 = nn.Conv2d(backbone_nb_channels, self.anchor_feat_channels, kernel_size=1, stride=1)    # output feature map: batch:8, chanel 512 -> 64, 11, 20 
        # self.cls_layer = nn.Linear(2 * self.anchor_feat_channels * self.fmap_h, 2)                          # 2*64*11 -> 2
        # self.reg_layer = nn.Linear(2 * self.anchor_feat_channels * self.fmap_h, self.n_offsets + 1)         # 2*64*11 -> S+1
        self.cls_layer = nn.Linear(2 * self.anchor_feat_channels, 2)                          # for non-anchor pooling, 2*64 -> 2
        self.reg_layer = nn.Linear(2 * self.anchor_feat_channels, self.n_offsets + 1)         # for non-anchor pooling, 2*64 -> S+1
        # self.attention_layer = nn.Linear(self.anchor_feat_channels * self.fmap_h, len(self.anchors) - 1)    # (chanel:64 * 11) -> 1000 (l+b+r)
        self.attention_layer = nn.Linear(self.anchor_feat_channels, len(self.anchors) - 1)    
        # for non-anchor pooling, (chanel:64 * 11) -> 1000 (l+b+r)
        # self.cls_layer = nn.Linear(2 * self.anchor_feat_channels, 2)
        # self.reg_layer = nn.Linear(2 * self.anchor_feat_channels, self.n_offsets + 1)
        # self.attention_layer = nn.Linear(self.anchor_feat_channels, len(self.anchors) - 1)
    ...
    def forward(self, x, conf_threshold=None, nms_thres=0, nms_topk=3000):
        batch_features = self.feature_extractor(x)  # 8, 512, 11, 20
        batch_features = self.conv1(batch_features) # 8, 64, 11, 20
        # batch_anchor_features = self.cut_anchor_features(batch_features)
        batch_anchor_features = self.cut_anchor_features(batch_features, self.anchors_cut) # for non-anchor pooling, return: # 8, 1000, 64

        # Join proposals from all images into a single proposals features batch
        # batch_anchor_features = batch_anchor_features.view(-1, self.anchor_feat_channels * self.fmap_h) # 8, 1000, 1, 64*11
        batch_anchor_features = batch_anchor_features.view(-1, self.anchor_feat_channels) 
        # for non-anchor pooling: 8, 1000, 64

        # Add attention features
        softmax = nn.Softmax(dim=1) # 8, 1000, 1, 11*64.  for non-anchor pooling: 8, 1000, 64
        scores = self.attention_layer(batch_anchor_features) # 8, 1000, 1000, 1.  for non-anchor pooling: 8, 1000, 1000
        attention = softmax(scores).reshape(x.shape[0], len(self.anchors), -1)  # 8, 1000, 1000
        attention_matrix = torch.eye(attention.shape[1], device=x.device).repeat(x.shape[0], 1, 1)  # 8, 1000, 1000 (eye: 1000)
        non_diag_inds = torch.nonzero(attention_matrix == 0., as_tuple=False)   # 8*(1000-1)*1000, 3
        attention_matrix[:] = 0
        attention_matrix[non_diag_inds[:, 0], non_diag_inds[:, 1], non_diag_inds[:, 2]] = attention.flatten() # 8, 1000, 1000, localed weights
        batch_anchor_features = batch_anchor_features.reshape(x.shape[0], len(self.anchors), -1) # 8, 1000, (64,*11)
        attention_features = torch.bmm(torch.transpose(batch_anchor_features, 1, 2),
                                       torch.transpose(attention_matrix, 1, 2)).transpose(1, 2)
                                       # bmm (8, 64*11, 1000) and (8, 1000, 1000)
                                       # for non-anchor pooling: (8, 64, 1000) and (8, 1000, 1000)
        # attention_features = attention_features.reshape(-1, self.anchor_feat_channels * self.fmap_h)    # 8*1000, 64*11 (global)
        attention_features = attention_features.reshape(-1, self.anchor_feat_channels)    # for non-anchor pooling: 8*1000, 64 (global)
        # batch_anchor_features = batch_anchor_features.reshape(-1, self.anchor_feat_channels * self.fmap_h) # 8*1000, 64*11 (local)
        batch_anchor_features = batch_anchor_features.reshape(-1, self.anchor_feat_channels) # 8*1000, 64 (local)
        # attention_features = attention_features.reshape(-1, self.anchor_feat_channels)
        # batch_anchor_features = batch_anchor_features.reshape(-1, self.anchor_feat_channels)
        batch_anchor_features = torch.cat((attention_features, batch_anchor_features), dim=1)   # 8*1000, 2*64*11.  for non-anchor pooling: 8*1000, 2*64

        # Predict
        cls_logits = self.cls_layer(batch_anchor_features)  # 8*1000, 2
        reg = self.reg_layer(batch_anchor_features)         # 8*1000, S+1

        # Undo joining
        cls_logits = cls_logits.reshape(x.shape[0], -1, cls_logits.shape[1]) # 8, 1000, 2
        reg = reg.reshape(x.shape[0], -1, reg.shape[1]) # 8, 1000, S+1

        # Add offsets to anchors
        reg_proposals = torch.zeros((*cls_logits.shape[:2], 5 + self.n_offsets), device=x.device) # 8, 1000, 5+S
        reg_proposals += self.anchors
        reg_proposals[:, :, :2] = cls_logits # bit 0, bit 1 = cls_logits; bit 2, bit 3 = start(x, y)
        reg_proposals[:, :, 4:] += reg  # bit 4:4+S+1 = reg
    ...
    def compute_anchor_cut_indices(self, n_fmaps, fmaps_w, fmaps_h):
        ...
        # indexing
        unclamped_xs = torch.flip((self.anchors_cut[:, 5:] / self.stride).round().long(), dims=(1,)) # feature map coordinates 1000*11
        unclamped_xs = unclamped_xs.unsqueeze(2) # 1000*11*none
        unclamped_xs = torch.repeat_interleave(unclamped_xs, n_fmaps, dim=0).reshape(-1, 1) # n_fmaps * [0: n_proposals], fmap_h, 1
        cut_xs = torch.clamp(unclamped_xs, 0, fmaps_w - 1)
        unclamped_xs = unclamped_xs.reshape(n_proposals, n_fmaps, fmaps_h, 1)
        invalid_mask = (unclamped_xs < 0) | (unclamped_xs > fmaps_w) # theoretically can not happens
        cut_ys = torch.arange(0, fmaps_h)
        cut_ys = cut_ys.repeat(n_fmaps * n_proposals)[:, None].reshape(n_proposals, n_fmaps, fmaps_h)
        cut_ys = cut_ys.reshape(-1, 1) # (fmaps_h) * n_proposals * n_fmaps, 1
        cut_zs = torch.arange(n_fmaps).repeat_interleave(fmaps_h).repeat(n_proposals)[:, None] # (fmaps_h*n_proposals)*n_fmaps, 1

        return cut_zs, cut_ys, cut_xs, invalid_mask

    # def cut_anchor_features(self, features):    # feature: 8, 64, 11, 20
    #     # definitions
    #     batch_size = features.shape[0]  # 8
    #     n_proposals = len(self.anchors) # 1000
    ...
    #         # before view: 64, 11, 1000
    #         # afer view: 1000, 64, 11, 1
    #         rois[self.invalid_mask] = 0
    #         batch_anchor_features[batch_idx] = rois # 8, 1000, 64, 11, 1

    #     return batch_anchor_features
    
    # For non-anchor pooling
    def cut_anchor_features(self, features, anchors_cut):    # feature: 8, 64, 11, 20; anchors_cut: 1000, 5+11
        # definitions
        batch_size = features.shape[0]  # 8
        n_proposals = len(self.anchors) # 1000
        n_fmaps = features.shape[1]     # 64
        batch_anchor_features = torch.zeros((batch_size, n_proposals, n_fmaps), device=features.device)
        # 8, 1000, 64

        # expand the border
        feature_line_left = features[:,:,:,0].reshape(batch_size, n_fmaps, -1)
        feature_line_right = features[:,:,:,features.shape[3]-1].reshape(batch_size, n_fmaps, -1)
        feature_line_bottom = features[:,:,features.shape[2]-1,:].reshape(batch_size, n_fmaps, -1)
        feature_line = torch.cat((feature_line_left, feature_line_bottom, feature_line_right), 2)

        # actual cutting
        # version 2.0: use torch operation with expanded border of feature map, instead of complex loops. It takes 4 items/s !!!
        cut_idx = anchors_cut[:,3].reshape(n_proposals).long()
        batch_anchor_features = feature_line[:,:,cut_idx].reshape(batch_size, n_proposals, n_fmaps)

        # version 1.0: use loops to find start poing feature, cannot take advantage of gpu's efficient matrix operation, taking 90 s/item !!!

        # for batch_idx, img_features in enumerate(features): # img_features: 64, 11, 20
        #     rois = torch.zeros((n_proposals, img_features.shape[0]))
        #     for i_fmap in range(n_fmaps):
        #         for anchor_num in range(n_proposals):
        #             # start point location in feature map, whose origin is located at left-up corner
        #             start_x = anchors_cut[anchor_num,3] # [0~1]
        #             start_y = 1-anchors_cut[anchor_num,2] # [0~1]
        #             start_x_idx = min(int(start_x * img_features.shape[2]), img_features.shape[2]-1)
        #             start_y_idx = min(int(start_y * img_features.shape[1]), img_features.shape[1]-1)
        #             # select start point feature as the whole anchor feature
        #             rois[anchor_num, i_fmap] = img_features[i_fmap, start_y_idx, start_x_idx]
        #             # 1000, 64
        #     # rois = rois.view(n_proposals, n_fmaps, 1, 1)
        #     batch_anchor_features[batch_idx] = rois # [0:8], 1000, 64

        return batch_anchor_features # 8, 1000, 64
    ...
    def generate_side_anchors(self, angles, nb_origins, x=None, y=None):
        ...
        n_anchors = nb_origins * len(angles) # left: 432; right: 432; bottom: 1920; total: 2784
        ...
        anchors = torch.zeros((n_anchors, 2 + 2 + 1 + self.n_offsets))  # n_anchors, 5+72
        anchors_cut = torch.zeros((n_anchors, 2 + 2 + 1 + self.fmap_h)) # n_anchors, 5+11
    ...
    def generate_anchor(self, start, angle, cut=False):
        ...
        anchor[2] = 1 - start_y
        # version 1.0, see "cut_anchor_features"
        # anchor[3] = start_x

        # version 2.0, see "cut_anchor_features"
        # for non-anchor pooling, anchor[3] save the feature index at the expanded border of feature map
        start_x_idx = min(int(start_x * self.fmap_w), self.fmap_w-1)
        start_y_idx = min(int(start_y * self.fmap_h), self.fmap_h-1)
        if start_x <= 0.001:
            anchor[3] = int(start_y_idx)
        elif start_x >= 0.999:
            anchor[3] = int(( self.fmap_w + 2*(self.fmap_h-1) ) - start_y_idx)
        else:
            anchor[3] = int(start_x_idx + self.fmap_h)
...
def get_backbone(backbone, pretrained=False):
    if backbone == 'resnet122':
        ...
        backbone = torch.nn.Sequential(*list(alexnet(pretrained=pretrained).children())[:-2])
        fmap_c = 256
        stride = 16
