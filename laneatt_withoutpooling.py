...
class LaneATT(nn.Module):
    ...
    def forward(self, x, conf_threshold=None, nms_thres=0, nms_topk=3000):
        ...
        batch_features = batch_features[:,:,0:11,:]
        batch_features = batch_features.reshape(-1, self.anchor_feat_channels * self.fmap_h)
        

        batch_features = torch.cat((batch_features,batch_features),dim=0)
